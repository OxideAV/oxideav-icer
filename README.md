...
## Fuzzing (round-131 addition)

A cargo-fuzz harness lives under `fuzz/`. The single
`decode_segment` target drives arbitrary fuzz-supplied bytes
through three decode-side entry points in one call:
`header::walk_segment` (the 12-byte segment header + variable-
length packet-header list, IPN 42-155 §IV), `parse_icer_metadata`
(the multi-segment cursor walk), and `parse_icer` (the full pixel
...
every call must *return* a `Result` — malformed input yields
`Err(IcerError::…)`, well-formed input yields `Ok`, and neither
path may panic / abort / integer-overflow / index out of bounds /
OOM.

Run locally with cargo-fuzz on nightly:

```
cd fuzz && cargo fuzz run decode_segment -- -max_total_time=60
```

The seed corpus under `fuzz/corpus/decode_segment/` ships five
small ICER streams generated by the crate's own encoder (the
clean-room wall bars seeding from any third-party reference
codec's test vectors). The CI shim
`.github/workflows/fuzz.yml` defers to the OxideAV org-level
reusable `crate-fuzz.yml` workflow for the daily 30-minute
budget. Baseline pinned at zero crashes after an initial
60-second local session (11 429 iterations);
`fuzz/artifacts/decode_segment/` empty.
